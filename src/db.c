...
const int rgSizeList[MAX_MEM_LIST] = {16,   32,   64,   128,   256,       1024,
...
void load_rooms(FILE *fp) {
  ...
  for (;;) {
    ...
    /* horrible hack */
    if (3000 <= vnum && vnum < 3400) SET_BIT(pRoomIndex->room_flags, ROOM_LAW);
      ...
      else if (letter == 'C') /* clan */
      {
        ...
        switch (locks) {
          case 0:
            break;
          case 1:
            pexit->rs_flags = EX_ISDOOR | EX_CLOSED;
            break;
          case 2:
            pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED;
            break;
          case 3:
            pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_NOPASS;
            break;
          case 4:
            pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF;
            break;
          case 5:
            pexit->rs_flags =
                EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF | EX_NOPASS;
            break;
        }
...
CHAR_DATA *create_mobile(MOB_INDEX_DATA *pMobIndex) {
  ...
  } else /* read in old format and convert */
  {
    ...
    for (i = 0; i < 3; i++) mob->armor[i] = interpolate(mob->level, 100, -100);
      ...
      case 'D':
        ...
        switch (pReset->arg3) {
          ...
          case 0:
            break;
          case 1:
            SET_BIT(pexit->rs_flags, EX_ISDOOR | EX_CLOSED);
            break;
          case 2:
  ...
  for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next) {
    ...
    switch (pReset->command) {
      ...
      case 'O':
        ...
        if (pRoom->area->nplayer > 0 || pObjIndex->count >= pReset->arg4) break;
      ...
      case 'E':
        ...
        if (pReset->command == 'E') equip_char(LastMob, pObj, pReset->arg3);
        last = TRUE;
        break;
...
void reset_area(AREA_DATA *pArea) {
  ...
  for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++) {
    if ((pRoom = get_room_index(vnum))) reset_room(pRoom);
  }
