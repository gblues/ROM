...
void fwrite_char(CHAR_DATA *ch, FILE *fp) {
  ...
  if (IS_NPC(ch)) {
    ...
    fprintf(fp, "HMVP %d %d %d\n", ch->pcdata->perm_hit, ch->pcdata->perm_mana,
            ch->pcdata->perm_move);
  ...
  for (paf = ch->affected; paf != NULL; paf = paf->next) {
    ...
    fprintf(fp, "Affc '%s' %3d %3d %3d %3d %3d %10d\n",
            skill_table[paf->type].name, paf->where, paf->level, paf->duration,
...
void fwrite_pet(CHAR_DATA *pet, FILE *fp) {
  ...
  if (pet->gold > 0) fprintf(fp, "Gold %ld\n", pet->gold);
  if (pet->silver > 0) fprintf(fp, "Silv %ld\n", pet->silver);
  ...
  if (pet->damroll != pet->pIndexData->damage[DICE_BONUS])
    fprintf(fp, "Dam  %d\n", pet->damroll);
  fprintf(fp, "ACs  %d %d %d %d\n", pet->armor[0], pet->armor[1], pet->armor[2],
          pet->armor[3]);
      ...
      case 'E':
        ...
        if (!str_cmp(word, "End")) {
          ...
          if (!fNest || (fVnum && obj->pIndexData == NULL)) {
            ...
            if (!obj->pIndexData->new_format && obj->item_type == ITEM_ARMOR &&
                obj->value[1] == 0) {
