...
void do_envenom(CHAR_DATA *ch, char *argument) {
  ...
  if (obj->item_type == ITEM_WEAPON) {
    if (IS_WEAPON_STAT(obj, WEAPON_FLAMING) ||
        ...
        IS_WEAPON_STAT(obj, WEAPON_SHOCKING) || IS_OBJ_STAT(obj, ITEM_BLESS) ||
...
void do_sacrifice(CHAR_DATA *ch, char *argument) {
  ...
  if (arg[0] == '\0' || !str_cmp(arg, ch->name)) {
    act("$n offers $mself to Mota, who graciously declines.", ch, NULL, NULL,
        TO_ROOM);
...
void do_zap(CHAR_DATA *ch, char *argument) {
  ...
  if (wand->value[2] > 0) {
    ...
    if (ch->level < wand->level ||
      ...
      act("Your efforts with $p produce only smoke and sparks.", ch, wand, NULL,
          TO_CHAR);
      act("$n's efforts with $p produce only smoke and sparks.", ch, wand, NULL,
          TO_ROOM);
...
void do_buy(CHAR_DATA *ch, char *argument) {
  ...
  if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
    ...
    if (cost <= 0 || !can_see_obj(ch, obj)) {
      act("$n tells you 'I don't sell that -- try 'list''.", keeper, NULL, ch,
          TO_VICT);
      ch->reply = keeper;
      return;
    }
    ...
    if (!IS_OBJ_STAT(obj, ITEM_INVENTORY)) {
      ...
      if (count < number) {
        act("$n tells you 'I don't have that many in stock.", keeper, NULL, ch,
            TO_VICT);
        ch->reply = keeper;
        return;
      }
    ...
    if ((ch->silver + ch->gold * 100) < cost * number) {
      if (number > 1)
        act("$n tells you 'You can't afford to buy that many.", keeper, obj, ch,
            TO_VICT);
      else
        act("$n tells you 'You can't afford to buy $p'.", keeper, obj, ch,
            TO_VICT);
      ch->reply = keeper;
      return;
    }
