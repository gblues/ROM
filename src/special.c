...
bool spec_guard(CHAR_DATA *ch) {
  ...
  if (victim != NULL) {
    snprintf(buf, sizeof(buf), "%s is a %s!  PROTECT THE INNOCENT!!  BANZAI!!", victim->name,
            crime);
    REMOVE_BIT(ch->comm, COMM_NOSHOUT);
    do_function(ch, &do_yell, buf);
    multi_hit(ch, victim, TYPE_UNDEFINED);
...
char *spec_string(SPEC_FUN *fun) /* OLC */
{
  ...
  for (cmd = 0; spec_table[cmd].function != NULL; cmd++)
    if (fun == spec_table[cmd].function) return spec_table[cmd].name;
...
SPEC_FUN *spec_lookup(const char *name) /* OLC */
{
  ...
  for (cmd = 0; spec_table[cmd].name[0] != '\0'; cmd++)
    if (!str_cmp(name, spec_table[cmd].name)) return spec_table[cmd].function;
