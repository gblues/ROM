...
REDIT(redit_ed) {
  ...
  if (!str_cmp(command, "edit")) {
    ...
    for (ed = pRoom->extra_descr; ed; ed = ed->next) {
      if (is_name(keyword, ed->keyword)) break;
    }
...
const struct wear_type wear_table[] = {{WEAR_NONE, ITEM_TAKE},
...
int wear_bit(int loc) {
  ...
  for (flag = 0; wear_table[flag].wear_loc != NO_FLAG; flag++) {
    if (loc == wear_table[flag].wear_loc) return wear_table[flag].wear_bit;
  }
...
OEDIT(oedit_delaffect) {
  ...
  {
    while ((pAf_next = pAf->next) && (++cnt < value)) pAf = pAf_next;
...
MEDIT(medit_position) {
  ...
  switch (arg[0]) {
    ...
    case 'S':
    case 's':
      if (str_prefix(arg, "start")) break;

      if ((value = position_lookup(argument)) < 0) break;
    ...
    case 'D':
    case 'd':
      if (str_prefix(arg, "default")) break;
