...
void say_spell(CHAR_DATA *ch, int sn) {
  ...
  static const struct syl_type syl_table[] = {
      ...
      {"tect", "infra"}, {"tri", "cula"},   {"ven", "nofo"},  {"a", "a"},
      {"b", "b"},        {"c", "q"},        {"d", "e"},       {"e", "z"},
      {"f", "y"},        {"g", "o"},        {"h", "p"},       {"i", "u"},
      {"j", "y"},        {"k", "t"},        {"l", "r"},       {"m", "w"},
      {"n", "i"},        {"o", "a"},        {"p", "s"},       {"q", "d"},
  ...
  for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
    if (rch != ch)
      act((!IS_NPC(rch) && ch->class == rch->class) ? buf : buf2, ch, NULL, rch,
          TO_VICT);
  }
...
bool saves_spell(int level, CHAR_DATA *victim, int dam_type) {
  ...
  if (!IS_NPC(victim) && class_table[victim->class].fMana) save = 9 * save / 10;
...
void do_cast(CHAR_DATA *ch, char *argument) {
  ...
  switch (skill_table[sn].target) {
    ...
    case TAR_OBJ_CHAR_OFF:
      ...
      if (target == TARGET_CHAR) /* check the sanity of the attack */
      {
        ...
        if (!IS_NPC(ch)) check_killer(ch, victim);
  ...
  if (number_percent() > get_skill(ch, sn)) {
    ...
    ch->mana -= mana;
    if (IS_NPC(ch) || class_table[ch->class].fMana) /* class has spells */
  ...
  if (target == TARGET_OBJ) {
    ...
    if (IS_OBJ_STAT(obj, ITEM_EVIL)) {
      ...
      if (!saves_dispel(level, paf != NULL ? paf->level : obj->level, 0)) {
        ...
        act("The evil of $p is too powerful for you to overcome.", ch, obj,
            NULL, TO_CHAR);
        return;
      }
    }
...
void spell_chain_lightning(int sn, int level, CHAR_DATA *ch, void *vo,
  ...
  act("A lightning bolt leaps from $n's hand and arcs to $N.", ch, NULL, victim,
      TO_ROOM);
  act("A lightning bolt leaps from your hand and arcs to $N.", ch, NULL, victim,
      TO_CHAR);
  ...
  if (target == TARGET_OBJ) {
    ...
    if (IS_OBJ_STAT(obj, ITEM_BLESS)) {
      ...
      if (!saves_dispel(level, paf != NULL ? paf->level : obj->level, 0)) {
        ...
        act("The holy aura of $p is too powerful for you to overcome.", ch, obj,
            NULL, TO_CHAR);
        return;
      }
    }
...
void spell_frenzy(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
  ...
  if (is_affected(victim, skill_lookup("calm"))) {
    if (victim == ch)
      ...
      act("$N doesn't look like $e wants to fight anymore.", ch, NULL, victim,
          TO_CHAR);
    return;
  }
        ...
        switch (obj_lose->item_type) {
          ...
          case ITEM_WEAPON:
            if (obj_lose->wear_loc != -1) /* try to drop it */
            {
              ...
              if (can_drop_obj(victim, obj_lose) &&
                  remove_obj(victim, obj_lose->wear_loc, TRUE)) {
                act("$n is burned by $p, and throws it to the ground.", victim,
...
void spell_summon(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
  ...
  if ((victim = get_char_world(ch, target_name)) == NULL || victim == ch ||
      victim->in_room == NULL || IS_SET(ch->in_room->room_flags, ROOM_SAFE) ||
      IS_SET(victim->in_room->room_flags, ROOM_SAFE) ||
      IS_SET(victim->in_room->room_flags, ROOM_PRIVATE) ||
      IS_SET(victim->in_room->room_flags, ROOM_SOLITARY) ||
      IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL) ||
...
void spell_weaken(int sn, int level, CHAR_DATA *ch, void *vo, int target) {
  ...
  if (is_affected(victim, sn) || saves_spell(level, victim, DAM_OTHER)) return;
