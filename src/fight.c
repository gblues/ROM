...
void mob_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt) {
  ...
  switch (number) {
    ...
    case (4):
      if (IS_SET(ch->off_flags, OFF_KICK_DIRT)) do_function(ch, &do_dirt, "");
      break;
...
void one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt) {
  ...
  if (result && wield != NULL) {
    ...
    if (ch->fighting == victim && IS_WEAPON_STAT(wield, WEAPON_FROST)) {
      ...
      act("The cold touch of $p surrounds you with ice.", victim, wield, NULL,
          TO_CHAR);
...
bool damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, int dam_type,
  ...
  if (victim->position == POS_DEAD) {
    ...
    if (!IS_NPC(victim)) {
      snprintf(log_buf, LOGBUF_SIZE, "%s killed by %s at %d", victim->name,
    ...
    snprintf(log_buf, LOGBUF_SIZE, "%s got toasted by %s at %s [room %d]",
...
bool is_safe_spell(CHAR_DATA *ch, CHAR_DATA *victim, bool area) {
  ...
  if (IS_NPC(victim)) {
    ...
    if (victim->pIndexData->pShop != NULL) return TRUE;
    ...
    if (!IS_NPC(ch)) {
      /* no pets */
      if (IS_SET(victim->act, ACT_PET)) return TRUE;
...
void do_bash(CHAR_DATA *ch, char *argument) {
  ...
  if (!IS_NPC(victim) &&
      chance <
          get_skill(victim,
                    gsn_dodge)) { /*
                                  act("$n tries to bash you, but you dodge
                                  it.",ch,NULL,victim,TO_VICT); act("$N dodges
                                  your bash, you fall flat on your
                                  face.",ch,NULL,victim,TO_CHAR);
  ...
  if (number_percent() < chance) {
    ...
    act("You evade $n's bash, causing $m to fall flat on $s face.", ch, NULL,
        victim, TO_VICT);
...
void do_kick(CHAR_DATA *ch, char *argument) {
  ...
  if (IS_NPC(ch) && !IS_SET(ch->off_flags, OFF_KICK)) return;
